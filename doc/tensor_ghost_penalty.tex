...
For load balancing we use a similar strategy to~\cite{bergbauer2024high} where to each cell a weight is assigned. For
...
interior cells and intersected cells. The cells are then redistributed among the processors in such a way that the sum
of the weights is equal for each processor.
...
cost, as shown in Figure~\ref{fig:vmult_time_breakdown}. Further optimizations, such as fusing the face loop with the
cell loop to reduce memory transfers, were considered but not implemented due to their complexity and the diminishing
returns as the mesh is refined and the fraction of stabilized faces decreases.

% \paragraph{Parallelization}
We use the distributed computing via MPI through infrastructure implemented in \texttt{deal.II}. The cells are
distributed among the processors and the operator is applied in parallel. The communication between the processors is
minimized by using the ghosted vectors, which store the values of the solution on the neighboring cells. The values of
the solution on the ghost cells are updated after each application of the operator. The main difference between
application of the operator considered in this work and the standard matrix-free approach is that the ghost penalty
operator is applied to the faces. This requires extending the data structures inside matrix-free infrastructure to
store information about the ghost cell.
