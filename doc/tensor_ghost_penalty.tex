...
$x_q$, we compute the gradient of the solution $\nabla u_K(x_q)$ and store it. Next, in the \textit{quadrature loop}
step, we prepare the data for integration by queuing the computed gradients $\nabla u_K(x_q)$. Finally, in the
\textit{integration} step, we compute the local contributions $w_{Ki}$ by performing the integration, which involves
contracting the stored gradients with test function gradients $\nabla\phi_i(x_q)$, Jacobian determinants $J(x_q)$, and
quadrature weights $\omega_q$. Both stages are performed efficiently using sum factorization, where the integration
step corresponds to the transpose of the evaluation operation~\cite{kopriva2009spectral, deville2002high}.
...
\begin{enumerate}
    \item \textbf{Evaluation of Solution Gradients:} Evaluate the solution values at each quadrature point $x_I$
          ...
          domain:
          \begin{align}
              \nabla	 u_K(x_I) & = \sum_i u_{K,i} \bfm{J}(x_I)^{-T}	\nabla \phi_i(x_I).
          \end{align}
    ...
    \item \textbf{Evaluation of Gradients and Values at the Boundary:} Evaluate the solution values at each
          ...
          \begin{align}
              ...
              \nabla   u_K(x_S) & = \sum_i u_{K,i} \bfm{J}^{-1}(x_S)	\nabla \phi_i(x_S).
          \end{align}
...
\begin{algorithm}[!ht]
    ...
    \ForEach{element $K \in \Omega^h$, $K \cap \Omega \neq \emptyset$  }{
        \If{$K\subset \Omega $	\tcp*{Handle as interior cell}	}{
            ...
            \Indp $w_{Ki} \leftarrow \sum_q \nabla {\phi}_i(x_q) \cdot \bfm{J}^{-T}(x_q) \nabla {u}_K(x_q) \, \, \omega_q$
            \tcp*{Sum factorization} \Indm Scatter results to ${w}$\\
        ...
        \ElseIf{$K \cap \partial \Omega^h \neq 0$  \tcp*{Handle as intersected cell} }{
            ...
            \Indp
            $w_{Ki} \leftarrow \sum_ {x_S}\nabla {\phi}_i(x_S) \cdot  \bfm{J}^{-T}(x_S) \nabla
                {u}_K(x_S) \,
                \, \omega(x_S) $
            \\
            \Indm
            ...
            gradients: \\ \Indp $w_{Ki} \stackrel{+}{\leftarrow} \sum_q \nabla {\phi}_i(x_q) \cdot \bfm{J}^{-T}(x_q) \nabla
                {u}_K(x_q) \, \, \omega_q$ \\ \Indm Scatter results to ${w}$\\ } } \ForEach{face $F \in \mathcal{F}_h^h$ \tcp*{Apply
            ghost penalty} }{ Gather face-local vector values on this face $u_F$\\ Evaluate local ghost penalty operator: \\ \Indp
        \(w_F \leftarrow \mathcal{G}_F u_F \) \\ \Indm Scatter results to ${w}$\\ } Import contributions $w$ \tcp*{Parallel
        communication} \caption{Matrix-free application of the CutFEM operator} \label{alg:mf_generic}
\end{algorithm}
